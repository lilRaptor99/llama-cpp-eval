...
| `JACCARD_SWEEP_SKIP`       | `0` (run)                                                                                              | Set to `1` to skip the per-cell Jaccard K-sweep post-processing block (`jaccard_sweep/`). The Python script itself is idempotent, so re-running the .sbatch re-renders any missing artefacts unless this is set.   |
...
plus a per-dataset bar chart at K = n_expert_used. Disable with
`JACCARD_SWEEP_SKIP=1`. The Python script itself is idempotent and
re-skips cells whose `summary.csv` already exists; pass `--force` to
override.
