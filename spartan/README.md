...
| `SKIP_PYTHON_PLOTS`        | `1` (skip all)                                                                                         | Master switch for every Python post-processing step run by the .sbatch. When `1` (default), the per-cell heatmap (`heatmap_from_cpp.py`), routing graph (`routing_graph_from_cpp.py`), and Jaccard K-sweep (`jaccard_sweep_from_cpp.py`) are all skipped — the C++ eval still writes its JSON, but no PNGs/CSVs are produced. Set to `0` to re-enable all three (matches the original behaviour, useful for visualisation passes after the eval is complete). The Python scripts are idempotent, so flipping back to `0` on a re-submit re-renders any missing artefacts. |
...
${RESULTS_DIR}/<model_safe>/<quant_safe>/jaccard_sweep/
...
├── summary.csv               # per-dataset Jaccard scalars
├── README.json               # meta: model, quant, K values, arch, token totals
└── jaccard_sweep.log         # stdout+stderr of jaccard_sweep_from_cpp.py
```

The `jaccard_sweep/` directory is generated by the
`spartan/llama-moe-eval.sbatch` post-processing step that invokes
[`examples/eval-moe-overview/jaccard_sweep_from_cpp.py`](../examples/eval-moe-overview/jaccard_sweep_from_cpp.py).
It compares each dataset's top-K experts against the
aggregated-across-datasets reference top-K (live-summed from the
per-dataset `expert_counts.json`s) and emits a K-sweep curve per layer
plus a per-dataset bar chart at K = n_expert_used. Disabled by default
via `SKIP_PYTHON_PLOTS=1` (the master switch for all Python
post-processing). Set `SKIP_PYTHON_PLOTS=0` to enable. The Python
script itself is idempotent and re-skips cells whose `summary.csv`
