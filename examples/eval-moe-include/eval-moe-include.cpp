...
struct moe_accumulator {
    ...
    // per (lang, dom): langdom -> vector<vector<vector<int64_t>>> [n_layer][n_expert][n_expert]
    // intra_pair_counts[key][L][e_i][e_j] = # tokens where both e_i and e_j fired
    //                                         in the top-k of layer L (k * k slots).
    std::map<std::string, std::vector<std::vector<std::vector<int64_t>>>> intra_counts;
    // per (lang, dom): langdom -> vector<vector<vector<int64_t>>> [n_layer - 1][n_expert][n_expert]
    // adj_pair_counts[key][L][e_i][e_j] = # tokens t such that e_i fired in layer L at t
...
// current (language, domain) bucket. Each token contributes k * k increments
// per layer pair (one for each (j1, j2) top-k slot pair). Must be called
// after every llama_decode() and before llama_memory_clear().
static void tally_pairs_for_question(moe_accumulator & acc) {
    ...
    const std::string & subj  = acc.current_langdom;
    ...
    for (int L = 0; L < n_layer; ++L) {
        if ((int) intra.size() <= L) {
            continue;  // langdom was skipped (no rows)
        }
...
int main(int argc, char ** argv) {
    ...
    // ---- aggregate: sum per-(lang, dom) matrices into a dataset-wide view.
    // Schema mirrors the per-langdom keys but uses `marginal_expert_counts`
    // for clarity (per-langdom keeps `layer_expert_counts` for backward
    ...
    for (const auto & kv : g_acc.counts) {
        ...
        const auto & intra_it = g_acc.intra_counts.find(key);
        ...
        const auto & adj_it = g_acc.adj_counts.find(key);
