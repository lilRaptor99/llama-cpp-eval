...
| `CUDA/12.8.0` + `Python/3.11.3` + `GCCcore/11.3.0` Lmod modules                                                            | Compiles + runs the C++ binaries    | Already pre-selected as defaults; override via `*_MODULE` env vars if your partition shows different versions. Note that `GCCcore/11.3.0` (the older compiler family) is required, not the newer `GCC/13.3.0`. |
...
# Default modules: CUDA/12.8.0, Python/3.11.3, GCCcore/11.3.0
