...
plus a props list, ready to be consumed by llama-eval-moe-popqa.
...
Outputs (default under build/moe-popqa/, override with --outdir):
  - popqa.jsonl     one row per question:
                    {"id", "subj", "prop", "obj", "s_pop", "o_pop",
...
def main() -> int:
    ...
    p.add_argument(
        "--outdir",
        default=str(Path(__file__).resolve().parents[2] / "build" / "moe-popqa"),
